...
export function DoctorEarnings({ earnings, payouts = [], nairaRate = 1000, perCreditEarning = 8, adminPercentage = 0 }) {
  ...
  const {
    ...
    availablePayout = 0,
    pendingCredits = 0,
    pendingPayout: pendingPayoutAmount = 0,
  } = earnings;

  const { loading, data, fn: submitPayoutRequest } = useFetch(requestPayout);
  const pendingPayoutRequest = payouts.find((payout) => payout.status === "PROCESSING");
  ...
  const grossAvailable = availableCredits * nairaRate;
  const platformFee = Math.max(grossAvailable - availablePayout, 0);
  const grossThisMonth =
    adminPercentage > 0 && adminPercentage < 100
      ? Math.round(thisMonthEarningsNaira / (1 - adminPercentage / 100))
    ...
    <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Gross {formatNaira(grossAvailable)} • Net {formatNaira(availablePayout)}
                </p>
                {pendingCredits > 0 && (
                  <p className="text-xs text-amber-400 mt-1">
                    {pendingCredits} credits (~{formatNaira(pendingPayoutAmount)}) pending from upcoming or unconfirmed appointments
                  </p>
                )}
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Gross {formatNaira(grossThisMonth)} • Net {formatNaira(thisMonthEarningsNaira)}
                </p>
              </div>
              <div className="bg-emerald-900/20 p-3 rounded-full">
      ...
      <Card className="border-emerald-900/20">
        ...
        <CardContent className="space-y-4">
            ...
            {pendingPayoutRequest ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3 text-sm">
                ...
                <div>
                  ...
                  <p className="text-white font-medium">{formatNaira(grossAvailable)}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">Estimated Platform Fee</p>
          ...
          <Alert>
            <AlertCircle className="h-4 w-4" />
            <AlertDescription className="text-sm"><strong>Payout Structure:</strong> You earn {formatNaira(perCreditEarning * nairaRate)} per credit after a {adminPercentage}% platform fee. Payouts include all your available credits and are processed after admin approval.</AlertDescription>
          </Alert>
          ...
          <form onSubmit={handlePayoutRequest} className="space-y-4">
            <div className="bg-muted/20 p-4 rounded-lg space-y-2">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white">{formatNaira(grossAvailable)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Platform fee:</span>
                <span className="text-white">{formatNaira(platformFee)}</span>
              </div>
              <div className="flex justify-between">
